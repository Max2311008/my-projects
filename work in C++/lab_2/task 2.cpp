...
int main() { //Объявляем главную функцию
	...
	// 2, 5, 5 строки обрабатывают кириллицу
	int height; // Создание переменной
	double length, width; 
	// Создание переменных с точкой, так как эти переменные могут иметь дробное значение
	double volume, squarefs;
	// Создание переменных с точкой, так как эти переменные могут иметь дробное значение
	std::cout << "Вычисление объёма параллелепипеда и площади поверхности" << std::endl;
	// Вывод текста на экран
	std::cout << "ДЛИНА ПАРАЛЛЕЛЕПИПЕДА = ";
	// Вывод текста на экран
	std::cin >> length;
	// Введение данных переменной с клавиатуры
	std::cout << "Число после округления = " << std::fixed << std::setprecision(2) << length << std::endl;
	// Округление числа до сотых
	std::cout << "ШИРИНА ПАРАЛЛЕЛЕПИПЕДА = ";
	// Вывод текста на экран
	std::cin >> width;
	// Введение данных переменной с клавиатуры
	...
	if (std::cin.fail() || length < 0 || width < 0 || height < 0) {
		...
		volume = static_cast<double>(length * width * height);
